...
#samples with village information:
# samples47 <- c("A9944","CHJ0007","CHJ0011",
...
# GeneticDistance47 <- as.dist(m[match(samples47,rownames(m)),match(samples50,rownames(m))]) #these numbers have to be changed to adjust to the number of samples
# save(GeneticDistance47, file="GeneticDistance47.Robj")
...
colors$HouseColor[colors$HouseColor=="white"] <- 'gray70'
  ...
  HCD %>% set("branches_col", c("brown3",rep("darkblue",3),rep("brown3",7),rep("darkblue",5),
    ...
    #plot(horiz = TRUE)
    circlize_dendrogram()
  legend("topright", fill = rev(unique(colors$CountryColor2)), legend = rev(levels(factor(colors$Country))),cex = 1, inset=c(-0.2,0), title = "\n Country \n (branches and leaves)", xjust = 0)
  ...
  colors$VillageColor2 = ifelse(colors$Village == "Santa_Ana", "gray62",
  ...
  #change house colors:
  colors$HouseColor = ifelse(colors$House == 'JUT-D-FM', 'deeppink4',
                      ifelse(colors$House == 'CHAP-D-65', 'magenta2',
                      ifelse(colors$House == 'CHAP-D-125', 'cyan',
                      #ifelse(colors$House == 'CHAP-D-155', 'cyan4',
                      ifelse(colors$House == 'CHAP-D-114-115', 'purple',
                      ifelse(colors$House == 'CHAP-D-54', 'dodgerblue4',
                      ifelse(colors$House == 'CHAP-D-64', 'deeppink2',
                      ...
                      ifelse(colors$House == 'CARR-D-39', 'deepskyblue',
                      ifelse(colors$House == 'CHAP-D-81', 'darkgreen',
                      ifelse(colors$House == 'CHAP-D-156', 'darkturquoise',
                      ifelse(colors$House == 'CARR-D-63', 'deeppink',
                      ifelse(colors$House == 'CARR-D-58', 'deeppink3',
                      ifelse(colors$House == 'CARR-D-57', 'orange',
                      ifelse(colors$House == 'CARR-D-95', 'darkorchid4',
                      ifelse(colors$House == 'CARR-D-93', 'firebrick1',
                      ifelse(colors$House == 'CARR-D-18A', 'blue',
                      #ifelse(colors$House == 'CHI-D19', 'royalblue4',
                      NA)))))))))))))))))))#)))
  
  colors$VillageIDs = ifelse(colors$Village == "Santa_Ana", "",
                                ifelse(colors$Village == "El Zacatal, Amatepec", "EZa",
                                ifelse(colors$Village == "El_Jute", "EJu",
                                ifelse(colors$Village == "El Aguacate, Azacualpa, Municipio de Armenia", "Aza",
                                ifelse(colors$Village == "Chilguyo, Texistepeque", "Chi",
                                ifelse(colors$Village == "Caserio Sarita Loma, Aldea La Primavera", "LPi",
                                ifelse(colors$Village == "Caserio San Raymundo, Canton Llano", "LDM",
                                ifelse(colors$Village == "Cantón Las Flores Cerro Alto, Municipio Caluco ", "CAl",
  ...
  colors$VillageLegend= ifelse(colors$Village == "Santa_Ana", "",
                        ifelse(colors$Village == "El Zacatal, Amatepec", "El Zacatal",
                        ifelse(colors$Village == "El_Jute", "El Jute",
                        ifelse(colors$Village == "El Aguacate, Azacualpa, Municipio de Armenia", "Azacualpa",
                        ifelse(colors$Village == "Chilguyo, Texistepeque", "Chilcuyo",
                        ifelse(colors$Village == "Caserio Sarita Loma, Aldea La Primavera", "La Primavera",
                        ifelse(colors$Village == "Caserio San Raymundo, Canton Llano", "Llano de Dona María",
                        ifelse(colors$Village == "Cantón Las Flores Cerro Alto, Municipio Caluco ", "Cerro Alto",
                        ifelse(colors$Village == "Llano_Santa_María", "Llano Santa María",
                        ifelse(colors$Village == "El_Chaperno", "El Chaperno",
                        ifelse(colors$Village == "El_Cerrón,_Olopa","El Cerrón",
                        ifelse(colors$Village == "La_Prensa,_Olopa","La Prensa",
                        ifelse(colors$Village == "El_Carrizal","El Carrizal",""
                        )))))))))))))
  ...
  colors$HouseColor[colors$HouseColor=="white"] <- 'gray62'
    ...
    #HCD <- set(HCD, "branches_col", c(rep(1,33),"aquamarine4",2,1,"aquamarine4",rep(1,100)))
    HCD %>% 
      set("branches_col", c(rep(1,7),1,rep(1,3),rep("darkgreen",3),rep(1,12),1,1,
                            rep(1,10),rep(1,7),rep(1,4),
                            1,1,rep("firebrick1",3),rep(1,1),rep("blue",4),
                            rep(1,100)
      )) %>%
      set("leaves_cex", 2) %>% 
      ...
      set("leaves_pch", c(rep(19,3),rep(17,2),rep(8,2),rep(19,3),
                          rep(17,4),rep(19,6),rep(17,3),
                          rep(8,6), rep(19,3),rep(17,4),rep(19,4),rep(17,6),
                          rep(19,4)
      )) %>%
      ...
      circlize_dendrogram()  
    #plot(horiz=TRUE)
    legend("topright",fill = unique(colors$VillageColor2)[c(1,2,13,10,11,7,5,6,4,8,12,9)], 
           legend = paste0(unique(colors$VillageLegend)[c(1,2,13,10,11,7,5,6,4,8,12,9)]," (",unique(colors$VillageIDs)[c(1,2,13,10,11,7,5,6,4,8,12,9)],")"),cex = 0.8, 
           inset=c(-0.4,-0.15), 
           title = "\n Village \n (leaves)", xjust = 0)
    
    legend("topright", fill = unique(colors$HouseColor)[order(unique(colors$House))[-1]], legend = unique(colors$House)[order(unique(colors$House))[-1]],cex = 0.8, 
           inset=c(-0.4,0.39), 
           title = "\n House (branches \n and labels)", xjust = 0)
    ...
    legend("topright",  legend = c("Same house genetically similar","Same village genetically similar"),
           cex = 0.8, 
           inset=c(-0.15,1.05), 
           title = "Symbols", xjust = 0,pch = c(8, 17))
