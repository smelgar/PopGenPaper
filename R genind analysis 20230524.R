...
#to get the mea and the sd in two tables:
...
# https://www.geeksforgeeks.org/how-to-color-scatter-plot-points-in-r/
#in a for loop, ggplot needs "print()": https://stackoverflow.com/questions/15678261/ggplot-does-not-work-if-it-is-inside-a-for-loop-although-it-works-outside-of-it
image = ggplot(dataDAPC, aes(x=LD1, y=LD2,group=Dept, shape=Villagepch))+ 
        geom_point(aes(color=Dept), shape=dataDAPC$Villagepch, cex=2.5, stroke = 1) + #stroke: https://stackoverflow.com/questions/51644418/how-to-change-the-linewidth-of-point-using-the-geom-point
        scale_color_manual(values = levels(dataDAPC$DeptCol))+ #https://stackoverflow.com/questions/15965870/fill-and-border-colour-in-geom-point-scale-colour-manual-in-ggplot
